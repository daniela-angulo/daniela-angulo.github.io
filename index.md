...
I have now a two summaries of my research. Check out the arxiv version of the <a href="https://arxiv.org/abs/2409.03680">experimental paper</a>. 

###Summary for everyone. 

 <p>Many of us learned in school that light travels more slowly in water or glass than in air or vacuum. If we measured the arrival time of a pulse of light passing through a piece of glass, we would observe a time delay known as the <em>group delay</em>.</p>
    <p>Light consists of individual particles called photons, which can interact with atoms. Sometimes, photons are absorbed by atoms, temporarily exciting them to higher energy states, and the atoms later release this energy as photons. This raises a natural question: when a pulse of light traverses a material, is its delay caused by photons being absorbed and then re-emitted by atoms? If so, how long does a photon spend "inside" the atoms? And is this time related to the total group delay observed for the light pulse? We decided to perform an experiment to answer these questions.</p>
      <p>In physics, measuring the value of a physical quantity requires interpreting it through its observable effects on the world; as the saying goes, <em>time is what a clock reads</em>. In our experiment, we measured how long atoms remained in an excited state, given that a photon passed through and was detected on the far side of the atom cloud. The <em>clock</em> we designed relied on an auxiliary beam that weakly interacted with the atoms. This beam experienced a change in its phase due to the atomic excitation caused by the single photon, providing the information needed to infer the "atomic excitation time''. Basically, the interaction between the single photon and the atomic medium subtly altered the medium, which, in turn, affected the propagation of the auxiliary beam.</p>
  
	<p>What we discovered is fascinating: transmitted photons can cause atoms to spend positive, zero, or even negative time in the excited state. Even more remarkably, we confirmed our recent theoretical prediction that this atomic excitation time matches the group delay (see <a href="https://arxiv.org/abs/2310.00432">theory</a>). But what does it mean for the group delay, or the atomic excitation time, to be negative?</p>
  
    <p>Let's begin by addressing the case of a negative group delay. When light is of a specific color -- one for which very little light gets through a material -- something strange can happen: the pulse of light can appear to exit the material before it entered. To illustrate this, imagine a scenario with Colombia’s <em>Túnel de la Línea</em>, a tunnel stretching 8.6 km through the Andes mountains. Cars traveling from Quindío (A) to Tolima (B) must pass through this tunnel. Suppose a convoy of 200 cars enters the tunnel, with the 100th car entering the tunnel at 12 noon. Partway through, construction workers halt traffic, allowing only 5 cars to exit the tunnel into Tolima. A camera at the exit records that the average exit time for these cars was 11:59 am— a minute before the cars entered! Of course, no individual car exited the tunnel before it entered. This strange result arises because we’ve compared the wrong averages: the average exit time for the few cars that got through with the average entrance time for all cars irregardless of whether they got through--the few cars that made it through aren’t representative of the entire group. Similarly, when a pulse of light travels through a medium, only a fraction of the incident light is transmitted. This can create a scenario where the peak of the <em>transmitted</em> pulse leaves the medium before the peak of the <em>incident</em> pulse enters it.</p>
    <p>Now let’s apply this analogy to our experiment and try to make sense of the negative atomic excitation time. Keep in mind that in quantum mechanics, the <em>cars</em> (photons) are indistinguishable, meaning we can't track which car is which.Instead of asking how long the average car spent in the tunnel, we focus only on the cars that successfully exited. How long were they inside? To answer this, we measure the impact they had on the tunnel—for instance, by checking the CO levels left behind by their engines. Surprisingly, in our analogy, the CO levels in the tunnel seem to <em>decrease</em> when we see a car on the other side of the tunnel, as though that car had produced a negative amount of CO. In fact, in the limiting case of just one car traversing the tunnel at a time, the measurement would yield a negative amount of CO inside the tunnel whenever the car exits. Of course, the notion of a "negative amount of CO" is nonsensical in the classical world, and this analogy breaks down. To understand such a result, we must enter the quantum realm.</p>

    <p>How did these <em>anomalous</em> results arise? In quantum mechanics, the more precisely you measure a system, the more you disturb it. To minimize the disturbance to the atoms, we deliberately designed an imprecise clock. This design made it impossible to determine whether a photon was transmitted by passing straight through the cloud without interacting with any atoms or by briefly exciting an atom and being re-emitted in the same direction. As a result, the auxiliary beam ended in a quantum superposition: one component indicating no time spent in the atom and another indicating a small positive time. The negative result arises from destructive interference between these two components—essentially subtracting <em>something positive</em> from <em>zero</em>.</p>
    <p>This experiment gives us new insights into the interaction between light and matter, offering a glimpse into the history of transmitted photons. We’ve shown that group delay not only tells us where the center of a light pulse is located but also accurately describes the magnitude—and even the sign—of the effect transmitted photons have on the atoms they interact with.</p>

###Summary for physics or related fields students. 
...
This <a href="https://www.arxiv.org/abs/quant-ph/0601134">work</a> demonstrates how to quantify and observe the effects of distinguishability in multiparticle photon states. The distinguishing information is encoded in hidden degrees of freedom, such as spectro-temporal modes, while the visible degree of freedom, in this case, is polarization. The paper reconstructs the state of two photons in polarization, illustrating how hidden information (e.g., arrival times) affects the visible degree of freedom.

The authors show that distinguishing information in arrival times can be inferred by performing tomographic measurements in polarization. The key insight is that the density matrix for the visible degree of freedom contains no coherences between the symmetric and antisymmetric subspaces. For two photons in polarization, this matrix is represented as a block diagonal form: a 3×3 block for the symmetric subspace and a single element for the antisymmetric subspace (singlet state).

To highlight the method's utility, the authors consider a known polarization state (e.g., HV) and reconstruct the visible density matrix under two conditions: (1) when the photons are indistinguishable, and (2) when a time delay greater than the photons' coherence length is introduced. In the first case, the population is entirely concentrated in the symmetric subspace (3×3 matrix). However, introducing the time delay shifts some population to the singlet state subspace, demonstrating the presence of hidden distinguishing information.
